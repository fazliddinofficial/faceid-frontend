...
function exportAttendanceToExcel(
  ...
  const rows = records.map((record) => ({
    ...
    Name: record.name,
    Date: formatDate(record.scanTime),
    Time: formatTime(record.scanTime),
    Type: record.attendanceType,
    "Verify Mode": record.verifyMode,
  }));
  ...
  XLSX.writeFile(workbook, `attendance-${startDate}-to-${endDate}.xlsx`);
}
    ...
    async function loadRecords() {
      try {
        const data = await getRange(
          getStartOfDayParam(startDate),
          getEndOfDayParam(endDate),
        );
        if (cancelled) return;
  ...
  return (
    <div>
      ...
      >
        ...
        <button
          onClick={() => exportAttendanceToExcel(filtered, startDate, endDate)}
          ...
          style={{
            ...
            fontWeight: 500,
            cursor:
              filtered.length === 0 || loading ? "not-allowed" : "pointer",
            opacity: filtered.length === 0 || loading ? 0.4 : 1,
          }}
        >
          ⬇ Download Excel
        </button>
